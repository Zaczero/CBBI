...
class TrolololoMetric(BaseMetric):
    ...
    def calculate(self, source_df: pd.DataFrame) -> pd.Series:
        ...
        # df['TroloOvershootModelValue'] = df['TroloTopPriceLog'] + df['TroloOvershootModel']
        # sns.set()
        # _, ax = plt.subplots()
        # sns.lineplot(x='Date', y='PriceLog', data=df, ax=ax)
        # sns.lineplot(x='Date', y='TroloTopPriceLog', data=df, ax=ax, color='lime')
        # sns.lineplot(x='Date', y='TroloBottomPriceLog', data=df, ax=ax, color='limegreen')
        # sns.lineplot(x='Date', y='TroloOvershootModelValue', data=df, ax=ax, color='red')
        # plt.show()
